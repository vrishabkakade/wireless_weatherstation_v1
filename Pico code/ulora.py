...
Customized by Vrishab Kakade specifically to send weather station data
"""
import time
import math
...
class ModemConfig:
    ...
    Bw125Cr48Sf4096 = (
        0x78, 0xc4, 0x0c)  # /< Bw = 125 kHz, Cr = 4/8, Sf = 4096chips/symbol, low data rate, CRC on. Slow+long range
...
class LoRa(object):
    ...
    def send(self, data, header_to, header_id=0, header_flags=0):
        ...
        header = [header_id]  # I only need to send header_id which is the count to make sure packets don't get dropped
        temp_data = []
        if type(data) == int:
            data = [data]
        ...
        # """
        # Below is the custom code written by Jag to go through a list of bytes and send them all as data
        # """
        elif type(data) == list:  # Assuming this list always contains a list of bytes
            for single_byte in data:
                temp = [p for p in single_byte]
                temp_data += temp
            data = temp_data
        elif type(data) == str:
        ...
        print("data being sent is", payload)
